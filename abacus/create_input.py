...
system_keys = [
        ...
        'kpar',                # devide all processors into kpar groups, and k points will be distributed among each group. 
        'bndpar',              # devide all processors into bndpar groups, and bands (only stochastic orbitals now) will be distributed among each group
        ...
        'mem_saver',           # Used only for nscf calculations. If set to 1, then a memory saving technique will be used for many k point calculations. 
        'diago_proc'           # If set to a positive number, then it specifies the number of threads used for carrying out diagonalization. 
        'nbspline'             # If set to a natural number, a Cardinal B-spline interpolation will be used to calculate Structure Factor.
]
# 2
file_keys = [
        ...
        'read_file_dir'        # when the program needs to read files such as electron density(SPIN1_CHG) as a starting point, this variables tells the location of the files. 
]
# 3
pw_keys = [
        ...
        'pw_diag_nmax',        # It indicates the maximal iteration number for cg/david method.
        'pw_diag_ndim'         # It indicates the maximal dimension for the Davidson method.
]
# 4
lcao_keys = [
        'nb2d',                # 2d distribution of atoms
        'lmaxmax',             # If not equals to 2, then the maximum l channels on LCAO is set to lmaxmax. If 2, then the number of l channels will be read from the LCAO data sets.
...
elec_keys = [
        ...
        'occupations',         # Specifies how to calculate the occupations of bands.
        ...
        'emin_sto',            # Trial energy to guess the lower bound of eigen energies of the Hamitonian Operator 
        'emax_sto',            # Trial energy to guess the upper bound of eigen energies of the Hamitonian Operator 
...
output_keys = [
        ...
        'out_freq_elec',            # it represents the frequency of electronic iters to output charge density and wavefunction
        'out_freq_ion',             # it represents the frequency of ionic iters to output charge density and wavefunction
        ...
        'out_alllog',               # determines whether to write log from all ranks in an MPI run. 
        ...
        'out_element_info',         # When set to 1, ABACUS will generate a new directory under OUT.suffix path named as element name
...
# 10
dipole_keys = [
        'efield_flag',              # If set to true, a saw-like potential simulating an electric field is added to the bare ionic potential.
        'dip_cor_flag',             # a dipole correction is also added to the bare ionic potential.
        'efield_dir',               # The direction of the electric field or dipole correction is parallel to the reciprocal lattice vector
        'efield_pos_max',           # Position of the maximum of the saw-like potential along crystal axis efield_dir
        'efield_pos_dec',           # Zone in the unit cell where the saw-like potential decreases
        'efield_amp'                # Amplitude of the electric field, in Hartree a.u.; 1 a.u. = 51.4220632*10^10 V/m. 
]
# 11
exx_keys = [
        ...
        'exx_c_threshold',          # Smaller components (less than exx_c_threshold) of the C matrix is neglected to accelerate calculation
        'exx_v_threshold',          # Smaller values of the V matrix can be truncated to accelerate calculation
        'exx_dm_threshold',         # Smaller values of the density matrix can be truncated to accelerate calculation.
        'exx_schwarz_threshold',    # using Cauchy-Schwartz inequality to find an upper bound of each integral before carrying out explicit evaluations.
        'exx_cauchy_threshold',     # using Cauchy-Schwartz inequality to find an upper bound of each integral before carrying out explicit evaluations.
        'exx_ccp_threshold',        # It is related to the cutoff of on-site Coulomb potentials, currently not used.
        'exx_ccp_rmesh_times',      # This parameter determines how many times larger the radial mesh required for calculating Columb potential is to that of atomic orbitals. 
        'exx_distribute_type',      # governs the mechanism of atom pairs' distribution
        'exx_opt_orb_lmax',         # The radial part of opt-ABFs are generated as linear combinations of spherical Bessel functions.   
        'exx_opt_orb_ecut',         # A plane wave basis is used to optimize the radial ABFs.     
        'exx_opt_orb_tolerence'     # determines the threshold when solving for the zeros of spherical Bessel functions.
...
dftu_keys = [
        ...
        'yukawa_potential',   # whether use the local screen Coulomb potential method to calculate the value of U and J
...
class AbacusInput:
    ...
    def write_input(self,
        ...
        write_abacus(join(self.directory, 'STRU'),
                    stru=atoms,
                    pseudo_dir=self.pseudo_dir,
                    pps=self.potential_name,
                    basis_dir=self.basis_dir,
                    basis=self.basis_name,
                    offsite_basis_dir=self.offsite_basis_dir,
                    offsite_basis=self.offsite_basis_name,
                    fix=self.fix,
                    scaled=scaled)
